...
function CustomTooltip({ active, payload, label }: {
  ...
  return (
    <div className="rounded-lg border border-neutral-700 bg-neutral-800 px-3 py-2 shadow-md text-xs">
      <p className="text-neutral-400 mb-0.5">{label ? formatDate(label) : ''}</p>
      <p className="font-semibold text-neutral-100">{formatNumber(payload[0].value)}</p>
    </div>
  )
}
    ...
    <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5">
      <div className="mb-4 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-2xl font-semibold text-white">{formatNumber(total)}</p>
        </div>
