...
export async function AssetTrafficBlock({ assetId }: { assetId: string }) {
  const supabase = await createServerSupabaseClient()
  const [stats, events] = await Promise.all([
    getAttributionStatsFor(supabase, { assetId }, PERIOD_DAYS),
    listAttributionFor(supabase, { assetId }, PERIOD_DAYS, 10),
  ])

  return (
    <section className="space-y-3">
      <div className="flex items-baseline justify-between">
        <h2 className="text-sm font-medium text-neutral-300">Trafic Umami (30j)</h2>
      ...
      {events.length === 0 ? (
        <p className="rounded-lg border border-dashed border-neutral-800 px-4 py-4 text-center text-xs text-neutral-500">
          Aucun clic Umami attribué à cet asset. Vérifie le Papermark URL ou ajoute une règle explicite.
        </p>
      ) : (
