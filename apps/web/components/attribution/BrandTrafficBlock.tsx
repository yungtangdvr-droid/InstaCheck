...
export async function BrandTrafficBlock({ brandId }: { brandId: string }) {
  const supabase = await createServerSupabaseClient()
  const [stats, events] = await Promise.all([
    getAttributionStatsFor(supabase, { brandId }, PERIOD_DAYS),
    listAttributionFor(supabase, { brandId }, PERIOD_DAYS, 10),
  ])
    ...
    <section className="space-y-3">
      ...
      {events.length === 0 ? (
        <p className="rounded-lg border border-dashed border-neutral-800 px-4 py-4 text-center text-xs text-neutral-500">
          Aucun clic attribué à cette brand. Ajoute une règle dans{' '}
