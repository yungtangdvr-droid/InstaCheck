...
import { createOpportunityTask, deleteDealTask, setDealTaskStatus } from '@/features/deals/actions'
import { formatDate } from '@/features/deals/utils'
...
export function DealTaskInline({
  tasks,
  linkedOpportunityId,
  linkedBrandId,
  linkedContactId,
  ...
  function add() {
    ...
    startTransition(async () => {
      const res = await createOpportunityTask({
        label,
  ...
  function toggle(task: Task) {
    ...
    startTransition(async () => {
      const res = await setDealTaskStatus(task.id, next)
  ...
  function remove(task: Task) {
    startTransition(async () => {
      const res = await deleteDealTask(task.id)
      if (res.error) setError(res.error)
      else router.refresh()
    })
  }
