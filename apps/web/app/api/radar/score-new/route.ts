// Manual Meme Radar — short scoring batch.
// Authenticated route called by the "Refresh Radar" button after
// /api/radar/ingest-now. Scores at most SCORE_LIMIT (5) candidates per
// request so the work fits well under the platform's request timeout.
// The button calls this endpoint repeatedly (capped client-side) to
// drain the backlog without ever holding a single long request open.
//
// Library-only: no shell-out. Mirrors the in-process inFlight + auth
// pattern used by /api/meta/sync-now and /api/content/analyze-new.
...
// Hard per-request cap. Do not raise without re-checking the platform
// timeout — the whole point of this route is to stay short.
const SCORE_LIMIT     = 5
...
export async function POST(_request: NextRequest) {
  ...
  if (inFlight) {
    return Response.json(
      { ok: false, error: 'Radar scoring already running' },
      { status: 409 },
    )
  }
  ...
  try {
    ...
    return Response.json(
      { ok: false, error: scoreError ?? 'score_failed' },
      { status: 500 },
    )
  } finally {
    inFlight = false
